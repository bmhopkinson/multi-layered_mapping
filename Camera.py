...
class Camera:
    ...
    def project(self, x_cam):
        ...
        BUFFER = 0.5
        z_pos = (x_cam[2] > 0)
        in_image_pinhole = (-1 * BUFFER * self.dim[0] < x_pinhole[0] < (1 + BUFFER) * self.dim[0]) and (
                    -1 * BUFFER * self.dim[1] < x_pinhole[1] < (1 + BUFFER) * self.dim[1])
        sanity_check = False
    ...
    def backproject(self, u, v, z):
        ...
        uc, vc = self.distortion_correction_oulu(ud,
                                                 vd)  # iteratively correct for radial distortion and tangential distortion
        #    pdb.set_trace()
